...
<section class="section">
    <div class="section-header justify-content-between">
        <h1> @lang('Manage Merchant')</h1>
        <form action="">
            <div class="row">
                <div class="col-md-6">
                    <select class="form-control" id="" onChange="window.location.href=this.value">
                        <option value="{{url('admin/manage-merchants/'.'?status=all')}}" {{request('status')=='all'
                            ?'selected':''}}>@lang('All')</option>
                        <option value="{{url('admin/manage-merchants/'.'?status=active')}}"
                            {{request('status')=='active' ?'selected':''}}>@lang('Active')</option>
                        <option value="{{url('admin/manage-merchants/'.'?status=banned')}}"
                            {{request('status')=='banned' ?'selected':''}}>@lang('Banned')</option>
                    </select>
                </div>
                <div class="col-md-6">
        ...
        <a href="{{route('admin.merchant.create')}}" class="btn btn-primary mb-1 my-2 mr-3"><i class="fas fa-plus"></i>
            @lang('Add New')</a>
    </div>
</section>
@endsection
                        ...
                        <tr>
                            ...
                            <td data-label="@lang('Action')">
                                <a class="btn btn-primary details"
                                    href="{{route('admin.merchant.details',$user->id)}}">@lang('Details')</a>
                            </td>
                            @endif
