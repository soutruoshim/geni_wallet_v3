...
@section('title')
   @lang(ucfirst($userType).' KYC Info')
@endsection

@section('breadcrumb')
 <section class="section">
    <div class="section-header justify-content-between">
        <h1>@lang(ucfirst($userType).' KYC Info')</h1>
        <form action="">
            <div class="row">
                <div class="col-md-6">
                    <select class="form-control" id="" onChange="window.location.href=this.value">
                        <option value="{{url('admin/kyc-info/'.$userType.'?status=pending')}}" {{request('status') == 'pending'?'selected':''}}>@lang('Pending')</option>
                        <option value="{{url('admin/kyc-info/'.$userType.'?status=approved')}}" {{request('status') == 'approved'?'selected':''}}>@lang('Approved')</option>
                        <option value="{{url('admin/kyc-info/'.$userType.'?status=rejected')}}" {{request('status') == 'rejected'?'selected':''}}>@lang('Rejected')</option>
                    </select>
                </div>
                <div class="col-md-6">
                            ...
                            <tr>
                                <td data-label="@lang('Sl')">{{$key + $userKycInfo->firstItem()}}</td>
                                 ...
                                 <td data-label="@lang('Details')">
                                     <a class="btn btn-info details" href="{{route('admin.kyc.details',[$userType,$info->id])}}">@lang('Details')</a>
                                 </td>
                                 @else
                                 <td>@lang('N/A')</td>
                                 @endif
